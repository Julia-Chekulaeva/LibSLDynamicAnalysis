...
class Graph1 {
    ...
    fun findOrCreateNextNode(prevNode: Node, methodCallLogInfo: MethodCallLogInfo): Node {
        ...
        var result = prevNode.children.keys.find { it.nodeInfo == nodeInfo }?.children?.keys?.find {
            it.name.matches(Regex("S\\d+"))
        }
        var currentPrevNode = prevNode
        if (result == null) {
            ...
            while (nodeBeforeCurrent != startNode) {
                ...
                if (nodeBeforeCurrent.nodeInfo == nodeInfo)
                    result = nodeBeforeCurrent.children.keys.find { it.name.matches(Regex("S\\d+")) }
                nodeBeforeCurrent = nodeBeforeCurrent.parent
                    ?: throw Exception("No parent node for node $nodeBeforeCurrent")
            }
            if (result == null) {
                result = Node(null, nodeInfo, currentPrevNode)
                nodes.add(result)
                connect(currentPrevNode, result)
                currentPrevNode = result
                result = Node("S${nextStateNumber++}", null, currentPrevNode)
                nodes.add(result)
            }
            connect(currentPrevNode, result)
        }
