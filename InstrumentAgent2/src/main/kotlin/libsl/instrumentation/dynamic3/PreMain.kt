...
import libsl.instrumentation.dynamic2.ClassFileMethodCallsTransformer
...
val config = File(libsl.instrumentation.dynamic2.configPath).readLines().associate { s ->
    val split = s.split("=")
    split[0] to split[1].removePrefix("\"").removeSuffix("\"")
}

val lslPath = libsl.instrumentation.dynamic2.config["lslPath"] ?: ""
val lslFileName = libsl.instrumentation.dynamic2.config["lslFileName"] ?: ""

fun premain(agentArgs: String?, instrumentation: Instrumentation) {
    val logger = Logger.getAnonymousLogger()
    logger.log(Level.INFO, "Instrumentation has started")
    addTransformer(instrumentation, libsl.instrumentation.dynamic2.lslFileName)
...
private fun addTransformer(instrumentation: Instrumentation, lslFileName: String) {
    ...
    val library = LibSL(libsl.instrumentation.dynamic2.lslPath).loadFromFileName(lslFileName)
